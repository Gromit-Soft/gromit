...
declare var $;
 ...
 namespace gromit {
     ...
     */
    function isUsed(usedCells: Cell[], col: number, row: number) : boolean {
     ...
     */
    function styleCell(cell: jQuery, x: number, y: number, cellWidth: number, cellHeight: number, animate: boolean): void {
        if (animate) {
            cell.animate({ 
                left: x + 'px',
                top: y + 'px'
    ...
    export function alignGrid(cWidth: number, cHeight: number, cPadding: number, cGridOffset: number) {
        cellWidth = cWidth;
        cellHeight = cHeight;
        padding = cPadding;
